...
const Doctors = () => {
  // 👉 In both frontend (React) and backend (Node/Express, etc.), params are used when we want to take dynamic values directly from the URL path.
  const { speciality } = useParams(); // when we want data from url then use params
  // pulls "speciality" from URL
  // backend const { speciality } = req.params;  // pulls from URL
  // Frontend (React) → useParams() from react-router-dom
